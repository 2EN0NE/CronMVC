<template>
  <div :class="{active: active}">
    <span v-for="(item, index) in cRange" :key="index">
      <input type="checkbox" v-model="cardInputData[index]">
      <label> {{index}} </label>
    </span>
  </div>
</template>
...
export default {
  ...
  computed: {
    cRange () {
      return Array(this.range[1] - this.range[0] + 1).fill(0).map((v, i) => i + this.range[0])
    },
  },
  methods: {
    reset () {
    ...
    mayResolve () {
      const result = this.hasSpecificChar(SPECIFIC_CHAR) || /^\d+$/.test(this.value);
      if( result ){
        this.active = true;
      }
      return result;
    ...
    validate () {
      ...
      chars.forEach(ele => {
        if(!/^\d+$/.test(ele)){
          this.errorMsg = 'the value that seperated by the "," have to be the positive integer!'
          return;
        }
        num =  parseInt(ele);
